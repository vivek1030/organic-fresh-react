...
import { Package, ChevronRight, ExternalLink, Clock, CheckCircle2, Truck } from 'lucide-react';
  ...
  if (loading) {
    return (
      <div className="container mx-auto px-4 py-32 text-center">
        ...
        <p className="mt-4 text-xs font-black uppercase tracking-widest text-stone-400">Loading Order History</p>
      </div>
    );
  }
      ...
      <div className="bg-stone-900 text-white py-16 md:py-24">
        <div className="container mx-auto px-4 md:px-6">
          ...
          <h1 className="text-5xl md:text-7xl font-black leading-[0.8]">
            Your <span className="text-green-500 italic font-serif">Harvest</span><br />
            History
          </h1>
        </div>
      </div>
        ...
        <div className="max-w-4xl">
          {orders.length === 0 ? (
            <div className="bg-white rounded-[2.5rem] p-16 text-center border border-stone-100 shadow-xl">
              ...
              <h3 className="text-2xl font-black text-stone-900 mb-2">No orders found</h3>
              <p className="text-stone-500 font-medium mb-8">You haven't placed any orders yet. Start your organic journey today!</p>
              <Link to="/shop">
                <Button className="px-10 py-6 font-black rounded-2xl">Start Shopping</Button>
              </Link>
            </div>
          ) : (
            <div className="space-y-6">
              {orders.map((order) => (
                <div key={order.id} className="bg-white rounded-[2.5rem] overflow-hidden border border-stone-100 shadow-sm hover:shadow-xl transition-all group">
                  <div className="p-6 md:p-8 flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-stone-50 bg-stone-50/30">
                    <div className="flex flex-wrap items-center gap-6">
                      ...
                      <div>
                        ...
                        <p className="text-sm font-black text-stone-900">{order.date}</p>
                      </div>
                      <div>
                        <p className="text-[10px] font-black uppercase tracking-widest text-stone-400 mb-1">Total</p>
                        <p className="text-sm font-black text-green-600">${order.total.toFixed(2)}</p>
                      </div>
                    </div>
                    <div className={`flex items-center gap-2 px-4 py-2 rounded-full border text-xs font-black uppercase tracking-widest w-fit ${getStatusStyles(order.status)}`}>
                      ...
                      {order.items.map((item) => (
                        <div key={item.id} className="flex items-center gap-4">
                          ...
                          <div className="flex-1 min-w-0">
                            <h4 className="text-sm font-black text-stone-900 truncate">{item.name}</h4>
                            <p className="text-xs font-bold text-stone-400 uppercase tracking-widest">Qty: {item.quantity}</p>
                          </div>
...
const Button = ({ children, className, ...props }: any) => (
  <button className={`bg-green-600 hover:bg-green-500 text-white transition-all shadow-xl shadow-green-100 ${className}`} {...props}>
    {children}
  </button>
);
