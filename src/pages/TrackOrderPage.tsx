    ...
    return (
      <div className="container mx-auto px-4 py-32 text-center">
        ...
        <p className="text-stone-500 mb-8">
          We couldn't find any record for ID: {id}
        </p>
        <Link to="/orders">
          ...
          <div className="lg:col-span-2 space-y-8">
                ...
                {order.tracking.map((step, idx) => (
                  ...
                  >
                    ...
                    {idx !== order.tracking.length - 1 && (
                      <div
                        className={`absolute left-[15px] top-[30px] bottom-0 w-0.5 ${step.isCompleted ? 'bg-green-600' : 'bg-stone-100'}`}
                      ></div>
                    )}
                    ...
                    <div className="flex-1 pt-1">
                      <div className="flex flex-col md:flex-row md:items-center justify-between gap-1 mb-2">
                        <h4
                          className={`text-lg font-black transition-colors ${step.isCurrent ? 'text-green-600' : 'text-stone-900'} ${!step.isCompleted && !step.isCurrent ? 'text-stone-300' : ''}`}
                        >
                          {step.status}
                        </h4>
                      ...
                      <p
                        className={`text-sm font-medium ${step.isCompleted || step.isCurrent ? 'text-stone-500' : 'text-stone-300'}`}
                      >
                        {step.date}
                      </p>
                    </div>
            ...
            <div className="bg-stone-900 rounded-[2.5rem] p-12 text-white relative overflow-hidden group">
              <div className="relative z-10 max-w-sm">
                <h3 className="text-3xl font-black mb-4 italic">
                  Real-Time Map View
                </h3>
                <p className="text-stone-400 font-medium mb-8 leading-relaxed">
                  Our delivery partner{' '}
                  <span className="text-white font-bold">Alex M.</span> is
                  currently 2.4 miles away from your location.
                </p>
          ...
          <div className="space-y-8">
            <div className="bg-white rounded-[2.5rem] p-8 border border-stone-100 shadow-sm">
              <h3 className="text-xl font-black text-stone-900 mb-6 flex items-center gap-2">
                <Package size={20} className="text-green-600" /> Delivery
                Details
              </h3>
            ...
            <div className="bg-green-50 rounded-[2.5rem] p-8 border border-green-100">
              <h3 className="text-xl font-black text-green-700 mb-4 italic">
                Safe Harvest Guarantee
              </h3>
              <p className="text-sm text-green-700/70 font-medium leading-relaxed mb-6">
                Your order is being transported in a temperature-controlled
                organic-certified vehicle to maintain maximum freshness.
              </p>
              <Link
                to="/shop"
                className="text-xs font-black uppercase tracking-widest text-green-700 flex items-center gap-2 hover:translate-x-1 transition-transform"
              >
